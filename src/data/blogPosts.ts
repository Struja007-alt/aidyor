...
export const blogPosts: BlogPost[] = [
  {
    ...
    content: `
      <h2>What Is a Honeypot Token?</h2>
      <p>A honeypot token is a fraudulent cryptocurrency designed to attract buyers while making it impossible — or extremely costly — to sell. The contract code contains hidden logic that allows purchases but blocks or taxes sales so heavily that withdrawing your investment is effectively impossible.</p>
      <p>Honeypot scams have cost crypto investors hundreds of millions of dollars. They are among the most common scams on decentralized exchanges (DEXs) like Uniswap, PancakeSwap, and Raydium.</p>

      <h2>How Do Honeypot Contracts Work?</h2>
      <p>The scammer deploys a smart contract that looks like a normal ERC-20 or BEP-20 token. Hidden inside the contract code are conditions that:</p>
      <ul>
        <li>Allow the deployer wallet to sell freely while blocking all other wallets from selling</li>
        <li>Set a hidden sell tax of 99–100%, draining your proceeds on exit</li>
        <li>Use a blacklist function to block specific addresses after purchase</li>
        <li>Modify transfer logic so sells revert silently with no error message</li>
      </ul>
      <p>Because DEX interfaces show real-time price charts and trading volume, honeypots can look identical to legitimate tokens — until you try to sell.</p>

      <h2>Warning Signs of a Honeypot Token</h2>
      <ul>
        <li><strong>Unverified smart contract</strong> — no source code visible on Etherscan or BscScan</li>
        <li><strong>Very high sell tax</strong> — any sell tax above 10% is a red flag; above 25% is almost always malicious</li>
        <li><strong>Concentrated holder distribution</strong> — top 10 wallets hold more than 50% of supply</li>
        <li><strong>No liquidity lock</strong> — developers can drain the pool instantly after your purchase</li>
        <li><strong>Anonymous team with no online presence</strong> — no verifiable founders or roadmap</li>
        <li><strong>Fake trading volume</strong> — wash trading between deployer wallets to simulate activity</li>
      </ul>

      <h2>How to Check for Honeypots Before Investing</h2>
      <p>The safest approach is to run a token through a honeypot checker before putting any money in. AIDYOR's free token scanner checks every token against multiple security APIs simultaneously, including:</p>
      <ul>
        <li>Live honeypot simulation — it actually attempts a simulated sell to verify the token is sellable</li>
        <li>Buy and sell tax analysis</li>
        <li>Contract verification status</li>
        <li>Holder concentration analysis</li>
        <li>Liquidity lock verification via Unicrypt, Team Finance, and PinkSale</li>
      </ul>
      <p>You can scan any token by contract address, name, or even by uploading a screenshot — AIDYOR's OCR engine extracts contract addresses automatically from images.</p>

      <h2>What to Do If You Suspect a Honeypot</h2>
      <p>If a scan returns a risk score below 40 or flags "Honeypot detected," do not invest. If you are already holding a suspected honeypot token, do not buy more hoping to average down — you will not be able to recover your investment. The only recourse is to report the contract address to the relevant blockchain explorer and warn others in the token's community.</p>

      <h2>Summary</h2>
      <p>Honeypot tokens are one of the most common and damaging crypto scams. Always scan a token with a free honeypot checker before investing, verify the contract source code, check liquidity lock status, and be suspicious of any token with a sell tax above 10%. AIDYOR gives you all of these checks in one place, in seconds, with no wallet connection required.</p>
    `
  },
  {
