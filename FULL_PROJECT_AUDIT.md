...
- **Supported Networks:** 8 (Ethereum, BNB Chain, Solana, Polygon, Arbitrum, Base, Optimism, Avalanche) — confirmed against live edge function source
- **Frontend hosting:** Vercel (auto-deploy from GitHub `main`)
- **Backend:** Supabase (sole backend, independently hosted)
...
1. Multi-Chain Token Scanner (8 networks)
...
- [x] ~~`.env` is committed to the public GitHub repo~~ — **fixed August 14, 2026.** `.env` deleted, `.gitignore` updated, `.env.example` added as a template.
- [ ] Leaked-password protection is disabled in Supabase Auth — **not actionable on the current Free plan** (this feature requires Supabase Pro and above). Deferred, not overlooked.
- [x] ~~`ocr-extract` returned a 401 on a real request~~ — **fixed August 13, 2026.** Root cause: `ocr-extract` and 6 other core scan functions (`risk-orchestrator`, `market-data-service`, `onchain-data-service`, `ai-risk-engine`, `ai-risk-explain`, `simulation-engine`) hard-required a logged-in Supabase user and 401'd otherwise — meaning anonymous visitors could never complete a scan, contradicting the "no signup required" positioning. All 7 functions were redeployed to treat an unresolved session as an anonymous visitor instead of rejecting the request. See `SECURITY_AUDIT_REPORT.md` Section 0 for full detail.
- [ ] Recommend a manual end-to-end test from an incognito tab to confirm the fix above works as intended in production.
