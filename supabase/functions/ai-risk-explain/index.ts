...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[ai-risk-explain] Request from ${user ? `user: ${user.id}` : 'anonymous visitor'}`);

    // ----- Pro gate: AI explanation is a paid feature -----
    let hasPro = false;
    if (user && user.email) {
      ...
      if (stripeKey) {
        try {
          ...
          console.error('[ai-risk-explain] Stripe check failed:', e);
        }
      }
    }
    ...
    if (!hasPro) {
      return new Response(JSON.stringify({
        error: "PRO_REQUIRED",
        message: "AI risk explanations are a Pro feature. Upgrade to unlock plain-English breakdowns for every scan.",
    ...
    if (!GEMINI_API_KEY) {
      console.error("[ai-risk-explain] GEMINI_API_KEY not configured");
...
${dangerFactors.length > 0 ? dangerFactors.map(f => `- ${f.name}: ${f.description}`).join("\n") : "- None detected"}

**Warnings (${warningFactors.length}):**
${warningFactors.length > 0 ? warningFactors.map(f => `- ${f.name}: ${f.description}`).join("\n") : "- None detected"}

**Positive Signals (${safeFactors.length}):**
${safeFactors.length > 0 ? safeFactors.map(f => `- ${f.name}: ${f.description}`).join("\n") : "- None detected"}
...
Provide a clear, actionable explanation of the risks.`;
    ...
    if (!response.ok) {
      ...
      console.error(`[ai-risk-explain] Gemini error: ${status}`);
    ...
    if (!explanation) {
      ...
      return new Response(JSON.stringify({ error: "Failed to generate explanation" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
