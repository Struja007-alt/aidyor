...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "start") {
      ...
      console.log(
        "Generated authentication options for email:",
        email
      );
    ...
    if (action === "complete") {
      ...
      const { data: storedCred, error: credError } =
        await supabaseAdmin
          .from("passkey_credentials")
          .select("*")
      ...
      return new Response(
        JSON.stringify({
          success: true,
          token,
          type: sessionData.properties.email_otp
            ? "otp"
            : "magiclink",
        }),
        {
    ...
    console.error(
      "[Internal] Passkey authentication error:",
      error
    );
