...
function checkRateLimit(identifier: string): {
  ...
  if (!entry || now >= entry.resetAt) {
    ...
    return {
      allowed: true,
      remaining: FREE_REQUESTS_PER_HOUR - 1,
      resetIn: hourMs,
    };
  }
  ...
  return {
    allowed: true,
    remaining: FREE_REQUESTS_PER_HOUR - entry.count,
    resetIn: entry.resetAt - now,
  };
}
...
async function checkWhaleProSubscription(
  supabase: any,
  userId: string
): Promise<boolean> {
  try {
...
serve(async (req) => {
  ...
  try {
    ...
    let minAmountUsd =
      typeof body.minAmountUsd === "number"
        ? body.minAmountUsd
        : 50000;
    ...
    // Enforce bounds
    minAmountUsd = Math.max(
      MIN_AMOUNT_RANGE.min,
      Math.min(
        MIN_AMOUNT_RANGE.max,
        minAmountUsd
      )
    );
    ...
    if (authHeader?.startsWith("Bearer ")) {
      ...
      if (
        ...
        isWhalePro =
          await checkWhaleProSubscription(
            serviceSupabase,
            userId
          );

        console.log(
          `[whale-alerts] User ${userId} - Whale Pro: ${isWhalePro}`
        );
      } else {
    ...
    if (!isWhalePro) {
      ...
      if (!rateLimit.allowed) {
        console.log(
          `[whale-alerts] Rate limited: ${rateLimitIdentifier}`
        );
        ...
        return new Response(
          JSON.stringify({
            ...
            message:
              "Free users are limited to 5 whale alert requests per hour. Upgrade to Whale Pro ($49/month) for unlimited access.",
            resetIn: Math.ceil(
              rateLimit.resetIn / 1000 / 60
            ),
      ...
      corsHeaders["X-RateLimit-Remaining"] =
        String(rateLimit.remaining);

      corsHeaders["X-RateLimit-Reset"] =
        String(
          Math.ceil(
            rateLimit.resetIn / 1000
          )
      ...
      corsHeaders["X-RateLimit-Limit"] =
        String(FREE_REQUESTS_PER_HOUR);
    } else {
    ...
    console.log(
      `[whale-alerts] Fetching with min amount: $${minAmountUsd}, limit: ${limit}, isWhalePro: ${isWhalePro}`
    );

    const whaleAlerts: WhaleAlert[] = [];

    // Fetch trending tokens
    const trendingTokens: Array<{
      chainId: string;
      tokenAddress: string;
    }> = [];
    ...
    for (const endpoint of TRENDING_ENDPOINTS) {
      try {
        ...
        console.log(
          "[whale-alerts] Failed to fetch trending data"
        );
      }
    }

    console.log(
      `[whale-alerts] Found ${trendingTokens.length} trending tokens to analyze`
    );
      ...
      trendingTokens
        ...
        .map(async (token) => {
          try {
            ...
            const hourlyVolume =
              mainPair.volume?.h1 || 0;

            const hourlyTxns =
              (mainPair.txns?.h1?.buys ||
                0) +
              (mainPair.txns?.h1?.sells ||
                0);
            ...
            if (
              ...
              // If average tx size suggests whale activity
              if (
                avgTxSize >=
                minAmountUsd * 0.5
              ) {
                // Estimate number of whale transactions
                const estimatedWhales =
                  Math.floor(
                    hourlyVolume /
                      minAmountUsd
                  );
            ...
            console.log(
              "[whale-alerts] Failed to analyze token"
            );
            return [];
          }
        });
    ...
    console.log(
      `[whale-alerts] Returning ${sortedAlerts.length} whale alerts`
    );
